...
const DEFAULT_PRIVATE_CHAIN_KIND: ChainKind = ChainKind::Private;
...
static PRIVATE_CHAIN_DEFAULT_ACCOUNTS: Lazy<Vec<GenesisAccount>> = Lazy::new(|| {
    ...
    let special_account = AdministratorAccount::new(
        DEFAULT_ADMIN_ACCOUNT_PUBLIC_KEY.clone(),
        Motes::new(SPECIAL_ACCOUNT_INITIAL_BALANCE),
    );
    let special_genesis_account = GenesisAccount::Administrator(special_account);
    default_accounts.push(special_genesis_account);
