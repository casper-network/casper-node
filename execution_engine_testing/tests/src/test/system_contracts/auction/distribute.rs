...
use casper_engine_test_support::{
    ...
    DEFAULT_ACCOUNT_ADDR, DEFAULT_GENESIS_TIMESTAMP_MILLIS, DEFAULT_PROTOCOL_VERSION,
...
fn should_distribute_delegation_rate_zero() {
    ...
    let genesis_round_seignorage_rate = Ratio::new(
        U512::from(*builder.get_initial_round_seignorage_rate().numer()),
        U512::from(*builder.get_initial_round_seignorage_rate().denom()),
    );
    let expected_total_reward = genesis_round_seignorage_rate * initial_supply;
    ...
    for _ in 0..=builder.get_initial_auction_delay() {
...
fn should_distribute_rewards_after_restaking_delegated_funds() {
    ...
    let expected_total_reward_1 = genesis_round_seignorage_rate * initial_supply;
    ...
    let expected_total_reward_2 = genesis_round_seignorage_rate * total_supply_2;
...
fn should_distribute_reinvested_rewards_by_different_factor() {
    ...
    let expected_total_reward_1 = genesis_round_seignorage_rate
        .checked_mul(&Ratio::from(initial_supply))
        .unwrap();
    let expected_total_reward_1_integer = expected_total_reward_1.to_integer();
...
fn should_distribute_delegation_rate_half() {
    ...
    let expected_total_reward = genesis_round_seignorage_rate
        .checked_mul(&Ratio::from(initial_supply))
        .expect("must get reward ratio");
    let expected_total_reward_integer = expected_total_reward.to_integer();
    assert_eq!(total_payout, expected_total_reward_integer);
...
fn should_distribute_delegation_rate_full() {
    ...
    assert_eq!(delegator_2_balance, delegator_2_expected_payout);
...
fn should_distribute_by_factor() {
    ...
    let expected_total_reward_integer = expected_total_reward.to_integer();
    assert_eq!(
        builder.base_round_reward(None),
        expected_total_reward_integer
    );
...
fn should_distribute_by_factor_regardless_of_stake() {
    ...
    let expected_total_reward = genesis_round_seignorage_rate
        .checked_mul(&Ratio::from(initial_supply))
        .unwrap();
    let expected_total_reward_integer = genesis_round_seignorage_rate
        .checked_mul(&Ratio::from(initial_supply))
        .map(|ratio| ratio.to_integer())
        .expect("must get reward as integer");
    assert_eq!(total_payout, expected_total_reward_integer);
...
fn should_distribute_delegation_rate_full_after_upgrading() {
    ...
    let expected_total_reward_before = genesis_round_seignorage_rate * initial_supply;
    ...
    let new_round_seigniorage_rate =
        builder.get_initial_round_seignorage_rate() * new_seigniorage_multiplier;
