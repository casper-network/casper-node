...
use casper_types::{
    ...
    Package, PackageAddr, RuntimeArgs, U512,
};
...
fn administrator_account_should_disable_any_contract_used_as_session() {
    ...
    let do_nothing_contract_package_hash =
        PackageAddr::new(do_nothing_contract_package_key.into_hash_addr().unwrap());
...
fn administrator_account_should_disable_any_contract_used_as_payment() {
    ...
    let test_payment_stored_package_hash = PackageAddr::new(addressable_entity.package().value());
