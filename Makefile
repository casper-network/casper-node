...
test-contracts-rs: build-contracts-rs
	...
	$(DISABLE_LOGGING) $(CARGO) test --release $(filter-out --release, $(CARGO_FLAGS)) -p casper-engine-tests -- --ignored repeated_ffi_call_should_gas_out_quickly
...
.PHONY: audit-rs
audit-rs:
	$(CARGO) audit --ignore RUSTSEC-2024-0003
