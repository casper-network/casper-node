...
pub(crate) trait ValidatorIdT: Eq + Ord + Clone + Debug + Hash + Send {}
impl<VID> ValidatorIdT for VID where VID: Eq + Ord + Clone + Debug + Hash + Send {}

/// The consensus value type, e.g. a list of transactions.
pub(crate) trait ConsensusValueT:
    Eq + Clone + Debug + Hash + Serialize + DeserializeOwned + Send
{
...
pub(crate) trait ValidatorSecret: Send {
    type Hash;
...
pub(crate) trait Context: Clone + DataSize + Debug + Eq + Ord + Hash + Send {
    ...
    /// A signature type.
    type Signature: Copy + Clone + Debug + Eq + Hash + Serialize + DeserializeOwned + Send;
