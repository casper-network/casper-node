...
use super::{
    gossip, one_way_messaging, peer_discovery, Config, Error, GossipMessage, OneWayCodec,
...
impl Behavior {
    pub(super) fn new(
        ...
        let (gossip_behavior, gossip_topic) =
            gossip::new_behavior(config, chainspec, our_public_key.clone())?;
    ...
    pub(super) fn gossip(&mut self, message: GossipMessage) {
        if let Err(error) = self
            .gossip_behavior
            .publish(self.gossip_topic.clone(), message)
        {
