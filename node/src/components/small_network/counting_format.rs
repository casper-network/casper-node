...
impl ConnectionId {
    ...
    fn create(random_data: TlsRandomData, our_id: NodeId, their_id: NodeId) -> ConnectionId {
        ...
        let mut id: [u8; Digest::LENGTH] = Digest::hash(&random_data.combined_random).into();
    ...
    fn create_trace_id(&self, flag: u8, count: u64) -> TraceId {
        ...
        let truncated = TryFrom::try_from(&full_hash.inner()[0..8]).expect("buffer size mismatch");
