...
struct ValidationContext {
    ...
    transactions_to_include: Vec<TransactionHashWithApprovals>,
...
impl ValidationContext {
    ...
    fn include_transactions<I: IntoIterator<Item = TransactionHashWithApprovals>>(
        mut self,
        transactions: I,
    ) -> Self {
...
async fn transfer_transaction_mixup_and_replay() {
    ...
    let mut context = ValidationContext::new()
        ...
        .include_all_transactions()
        .include_all_transfers()
        .include_transactions(vec![TransactionHashWithApprovals::from(&deploy_legacy)]);
    assert!(!context.validate_block(&mut rng, timestamp).await);
    let transactions = vec![deploy_legacy.clone(), transaction_v1.clone()];
    let transfers = vec![transfer_legacy.clone(), transfer_v1.clone()];
    let mut context = ValidationContext::new()
        ...
        .include_all_transactions()
        .include_all_transfers()
        .include_transactions(vec![TransactionHashWithApprovals::from(&transaction_v1)]);
    assert!(!context.validate_block(&mut rng, timestamp).await);
    ...
    let mut context = ValidationContext::new()
        ...
        .include_all_transactions()
        .include_all_transfers()
        .include_transfers(vec![TransactionHashWithApprovals::from(&transfer_v1)]);
    assert!(!context.validate_block(&mut rng, timestamp).await);
    let transactions = vec![deploy_legacy.clone(), transaction_v1.clone()];
    let transfers = vec![transfer_legacy.clone(), transfer_v1.clone()];
    let mut context = ValidationContext::new()
        ...
        .include_all_transactions()
        .include_all_transfers()
        .include_transfers(vec![TransactionHashWithApprovals::from(&transfer_legacy)]);
    assert!(!context.validate_block(&mut rng, timestamp).await);
}
