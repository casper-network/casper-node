...
impl LinearChain {
    ...
    pub(super) fn handle_put_block(
        ...
        if let Some(key_block_info) =
            KeyBlockInfo::maybe_from_block_header(block.header(), block_hash_v2_activation)
        {
...
mod tests {
    ...
    fn new_block_no_sigs() {
        ...
        let block_stored_outcomes =
            lc.handle_put_block(Box::new(block.clone()), block_hash_v2_activation);
    ...
    fn new_block_bonded_pending_sigs() {
        ...
        let outcomes = lc.handle_put_block(block.clone(), block_hash_v2_activation);
    ...
    fn new_block_then_own_sig() {
        ...
        let put_block_outcomes = lc.handle_put_block(block.clone(), block_hash_v2_activation);
        // Verify that all outcomes are expected.
    ...
    fn upgrade_when_fully_signed() {
        ...
        assert_equal(
            ...
            lc.handle_put_block(block, block_hash_v2_activation),
        );
