...
use crate::{
    ...
    reactor::Message as ReactorMessage,
...
pub trait ReactorEventT<T>:
    ...
    + From<NetworkRequest<NodeId, ReactorMessage>>
...
impl<T: Item + 'static, REv: ReactorEventT<T>> Gossiper<T, REv> {
    ...
    fn check_get_from_peer_timeout(
        ...
        match self.table.remove_holder_if_unresponsive(&item_id, peer) {
            ...
            GossipAction::GetRemainder { holder } => {
                ...
                // response.
                let request = match ReactorMessage::new_get_request::<T>(&item_id) {
                    Ok(request) => request,
                    Err(error) => {
    ...
    fn got_from_holder(
        ...
        match ReactorMessage::new_get_response(&item) {
