...
pub enum Transform {
    ...
    AddUInt256(U256),
    AddUInt512(U512),
    #[data_size(skip)] // TODO! Why can't we size Key?
...
impl From<&EngineTransform> for Transform {
    fn from(transform: &EngineTransform) -> Self {
        match transform {
            ...
            EngineTransform::AddKeys(named_keys) => Transform::AddKeys(named_keys.clone()),
...
impl FromBytes for Transform {
    fn from_bytes(bytes: &[u8]) -> Result<(Self, &[u8]), bytesrepr::Error> {
        ...
        match tag {
            ...
            ADD_KEYS_TAG => {
                let (value, remainder) = BTreeMap::<String, Key>::from_bytes(remainder)?;
...
mod tests {
    use casper_types::{
        ...
        DeployHash, HashAddr, TransferAddr, URef,
    };
        ...
        pub fn random(rng: &mut TestRng) -> Self {
            match rng.gen_range(0, 13) {
                ...
                11 => {
                    ...
                    map.insert("uref".to_string(), Key::URef(URef::default()));
                    map.insert(
                        "account".to_string(),
                        Key::Account(AccountHash::new([rng.gen::<u8>(); ACCOUNT_HASH_LENGTH])),
                    );
                    map.insert("hash".to_string(), Key::Hash(HashAddr::default()));
                    map.insert(
                        "deploy-info".to_string(),
                        Key::DeployInfo(DeployHash::default()),
                    );
                    map.insert(
                        "transfer".to_string(),
                        Key::Transfer(TransferAddr::default()),
                    );
