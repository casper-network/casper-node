...
#[cfg(test)]
mod tests {
    use super::{BinaryPayload, BinaryPayloadBuilder, Field};
    use crate::bytesrepr::*;

    #[test]
    fn binary_payload_should_serialize_fields() {
        let binary_payload = BinaryPayloadBuilder::new(vec![
        ...
        let bytes = binary_payload
            ...
            .unwrap();
        let (payload, remainder) = BinaryPayload::from_bytes(3, &bytes).unwrap();
    ...
    fn binary_payload_should_fail_to_deserialzie_if_more_then_expected_fields() {
        ...
        let bytes = binary_payload
            ...
            .binary_payload_bytes()
            .unwrap();
        let res = BinaryPayload::from_bytes(2, &bytes);
        assert!(res.is_err())
    }
    ...
    {
        let binary_payload =
            BinaryPayloadBuilder::new(vec![U8_SERIALIZED_LENGTH, U16_SERIALIZED_LENGTH]).unwrap();
        let res = binary_payload
            .add_field(0, &(254_u8))
            .unwrap()
            .add_field(1, &(u32::MAX));
