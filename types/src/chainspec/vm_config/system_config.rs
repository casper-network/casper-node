...
impl SystemConfig {
    ...
    pub fn random(rng: &mut TestRng) -> Self {
        ...
        let mut auction_costs: AuctionCosts = rng.gen();
        let mint_costs = rng.gen();
        let handle_payment_costs = rng.gen();
        let standard_payment_costs = rng.gen();

        // workaround for the test flakiness issue mentioned above
        auction_costs.change_bid_public_key = rng.gen::<u32>() as u64;
