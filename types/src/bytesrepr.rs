...
/// A writer trait that abstracts away serialization format.
pub trait Writer {
    /// Writes a chunk of raw bytes.
    fn write_bytes(&mut self, bytes: impl AsRef<[u8]>) -> Result<(), Error>;
    /// Writes an u8 byte to a writer.
...
impl<T: ToBytes> ToBytes for Vec<T> {
    ...
    where
        W: Writer,
    {
        writer.write_bytes(&(self.len() as u32).to_le_bytes())?;
        for item in self.iter() {
    ...
    ($($N:literal)+) => {
        $(
            impl ToBytes for [u8; $N] {
                ...
                #[inline(always)]
                fn write_bytes<W>(&self, writer: &mut W) -> Result<(), Error> where W: Writer {
...
impl<T: ToBytes> ToBytes for Option<T> {
    ...
    {
        match self {
            None => writer.write_u8(OPTION_NONE_TAG)?,
            Some(v) => {
                writer.write_u8(OPTION_SOME_TAG)?;
...
impl<T: ToBytes, E: ToBytes> ToBytes for Result<T, E> {
        ...
        W: Writer,
    {
        match self {
            Err(error) => {
                writer.write_u8(RESULT_ERR_TAG)?;
                error.write_bytes(writer)?;
            }
            Ok(result) => {
                writer.write_u8(RESULT_OK_TAG)?;
...
fn write_u8_slice<W>(bytes: &[u8], writer: &mut W) -> Result<(), Error>
where
    W: Writer,
{
