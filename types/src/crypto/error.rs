...
use core::fmt::Debug;
#[cfg(not(any(feature = "crypto_std", test)))]
...
#[derive(Debug)]
#[cfg_attr(any(feature = "crypto_std", test), derive(Error))]
pub enum Error {
    /// Error resulting from creating or using asymmetric key types.
    #[cfg_attr(any(feature = "crypto_std", test), error("asymmetric key error: {0}"))]
    AsymmetricKey(String),

    /// Error resulting when decoding a type from a hex-encoded representation.
    #[cfg_attr(any(feature = "crypto_std", test), error("parsing from hex: {0}"))]
    FromHex(base16::DecodeError),

    /// Error resulting when decoding a type from a base64 representation.
    #[cfg_attr(any(feature = "crypto_std", test), error("decoding error: {0}"))]
    FromBase64(base64::DecodeError),

    /// Signature error.
    #[cfg_attr(any(feature = "crypto_std", test), error("error in signature"))]
    SignatureError(SignatureError),

    /// Error trying to manipulate the system key.
    #[cfg_attr(
        any(feature = "crypto_std", test),
