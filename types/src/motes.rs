...
impl Motes {
    /// Maximum value a [`Motes`] instance can represent.
    ...
    pub fn from_gas(gas: Gas, conv_rate: u64) -> Option<Self> {
        let gas_u512 = U512::from(gas);
        gas_u512.checked_mul(U512::from(conv_rate)).map(Self)
    }
}
...
impl From<Motes> for U512 {
    fn from(motes: Motes) -> Self {
        motes.0
    }
}
...
mod tests {
    ...
    fn should_be_able_to_get_instance_of_motes() {
        ...
        assert_eq!(initial_value, motes.0.as_u64(), "should have equal value")
    }
    ...
    fn should_default() {
        ...
        assert_eq!(left_motes, right_motes, "should be equal");
        let u512 = U512::zero();
        assert_eq!(left_motes.0, u512, "should be equal");
    }
    ...
    fn should_support_checked_mul_from_gas() {
        let gas = Gas::MAX;
        let conv_rate = 10;
        let motes = Motes::from_gas(gas, conv_rate).expect("should calculate motes from gas");
        assert!(motes > Motes::new(U512::from(u64::MAX)));
    }
}
