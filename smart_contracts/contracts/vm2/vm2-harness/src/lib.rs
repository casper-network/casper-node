...
use casper_executor_wasm_common::{
    keyspace::{CollectionAddrInner, ContextAddr},
    type_uid::Uid,
    CollectionTypeTag,
};
...
fn perform_test(seed: &mut Seed, flipper_address: Address) {
...

    {
...

        {
            ...
            );
            {
                ...
                token_owner
                    .call(|contract| {
                        contract.set_fallback_handler(FallbackHandler::RejectWithRollback)
                    })
                    .expect("Should call");
                ...
                assert_eq!(
                    ...
                    Err(
                        crate::contracts::token_owner::TokenOwnerError::WithdrawError(
                            "deposit error callee rolled back".to_string()
                        )
                    )
                );
            }
    ...
    {
        next_test(&mut counter, "Getting typedefs");

        let expected_uid = Uid::from_name("u64");
        ...
        match typedef_bytes {
            ...
            None => casper::revert("Expected to find typedef for u64"),
        }
    }
