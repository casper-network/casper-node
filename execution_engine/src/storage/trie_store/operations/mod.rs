            ...
            Trie::Node { pointer_block } => {
                ...
                let indexed_pointers_with_hole = pointer_block
                    ...
                    .filter_map(|(index, pointer)| {
                        if index as usize != hole_index {
                            Some((index, pointer.to_owned()))
                        } else {
                            None
                        }
                    })
    ...
    E: From<S::Error> + From<bytesrepr::Error>,
{
    ...
    while let Some(trie_key) = trie_keys_to_visit.pop() {
        ...
        if let Some(trie_value) = &maybe_retrieved_trie {
            let hash_of_trie_value = trie_value.hash_digest()?;
    ...
    while let Some((idx, parent)) = parents.pop() {
        match (new_elements.last_mut(), parent) {
            ...
            (Some((trie_key, Trie::Node { .. })), Trie::Extension { affix, .. }) => {
                ...
                let trie_key = trie_extension.hash_digest()?;
    ...
    V: ToBytes + Clone,
{
    ...
    let mut tip_hash = tip.hash_digest()?;
    ...
    V: ToBytes,
{
    ...
    // to parents.
    if !affix.is_empty() {
        let new_node_hash = new_node.hash_digest()?;
    ...
    V: ToBytes + Clone,
{
    ...
    let maybe_hashed_child_extension: Option<(Digest, Trie<K, V>)> =
        if child_extension_affix.is_empty() {
            ...
            let child_extension_hash = child_extension.hash_digest()?;
...
where
    ...
    E: From<S::Error> + From<bytesrepr::Error>,
{
    let trie_hash = trie.hash_digest()?;
