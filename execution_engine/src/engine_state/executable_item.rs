...
use casper_types::{
    ...
    AddressableEntityHash, AddressableEntityIdentifier, EntityAddr, ExecutableDeployItem, Gas,
    Motes, PackageIdentifier, Phase, PublicKey, RuntimeArgs, Transaction,
    ...
    fn sample<R: Rng + ?Sized>(&self, rng: &mut R) -> ExecutableItem {
        ...
        match rng.gen_range(0..5) {
            ...
            5 => ExecutableItem::ByAddress {
                address: EntityAddr::new_with_tag(rng.gen(), rng.gen()),
                entry_point: random_string(rng),
                args,
            },
            _ => unreachable!(),
