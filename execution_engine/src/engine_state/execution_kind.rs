...
use crate::{
    engine_state::{error::Error, executable_item::ExecutableItem},
    execution::ExecError,
};
...
impl ExecutionKind {
    ...
    pub fn from_executable_deploy_item<R>(
        ...
        R: StateReader<Key, StoredValue, Error = GlobalStateError>,
    {
        ...
        match executable_deploy_item {
            ExecutableDeployItem::Transfer { .. } => {
                Err(Error::InvalidItemVariant("Transfer".into()))
            }
            ExecutableDeployItem::ModuleBytes { module_bytes, .. }
                if module_bytes.is_empty() && is_payment_phase =>
            {
                Err(Error::InvalidItemVariant(
    ...
    /// Returns all the details necessary for execution.
    ///
    /// This object is generated based on information provided by [`ExecutableItem`].
    pub fn from_executable_item<R>(
        tracking_copy: Rc<RefCell<TrackingCopy<R>>>,
        named_keys: &NamedKeys,
        ...
        R: StateReader<Key, StoredValue, Error = GlobalStateError>,
    {
        ...
        match executable_item {
            ...
            ExecutableItem::ByAddress {
                ...
                let entity_hash = AddressableEntityHash::new(address.value());
                // TODO: we should be using entity addr, no enity hash.
                // let entity_key = Key::AddressableEntity(address);
                Ok(ExecutionKind::new_addressable_entity(
                    entity_hash,
                    entry_point,
                ))
