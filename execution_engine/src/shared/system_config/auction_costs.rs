...
enum AuctionCostsKeys {
    GetEraValidators = 100,
    ReadSeigniorageRecipients = 101,
    AddBid = 102,
    WithdrawBid = 103,
    Delegate = 104,
    Undelegate = 105,
    ...
    Slash = 107,
    Distribute = 108,
    WithdrawDelegatorReward = 109,
    WithdrawValidatorReward = 110,
    ReadEraId = 111,
    ActivateBid = 112,
}
...
impl ToBytes for AuctionCosts {
    fn to_bytes(&self) -> Result<Vec<u8>, bytesrepr::Error> {
        ...
        writer.write_pair(AuctionCostsKeys::GetEraValidators, self.get_era_validators)?;
        writer.write_pair(
            AuctionCostsKeys::ReadSeigniorageRecipients,
            self.read_seigniorage_recipients,
        )?;
        writer.write_pair(AuctionCostsKeys::AddBid, self.add_bid)?;
        writer.write_pair(AuctionCostsKeys::WithdrawBid, self.withdraw_bid)?;
        writer.write_pair(AuctionCostsKeys::Delegate, self.delegate)?;
        writer.write_pair(AuctionCostsKeys::Undelegate, self.undelegate)?;
        writer.write_pair(AuctionCostsKeys::RunAuction, self.run_auction)?;
        writer.write_pair(AuctionCostsKeys::Slash, self.slash)?;
        writer.write_pair(AuctionCostsKeys::Distribute, self.distribute)?;
        writer.write_pair(
            AuctionCostsKeys::WithdrawDelegatorReward,
            self.withdraw_delegator_reward,
        )?;
        writer.write_pair(
            AuctionCostsKeys::WithdrawValidatorReward,
            self.withdraw_validator_reward,
        )?;
        writer.write_pair(AuctionCostsKeys::ReadEraId, self.read_era_id)?;
        writer.write_pair(AuctionCostsKeys::ActivateBid, self.activate_bid)?;
...
impl FromBytes for AuctionCosts {
    fn from_bytes(bytes: &[u8]) -> Result<(Self, &[u8]), casper_types::bytesrepr::Error> {
        let mut auction_costs = AuctionCosts::default();

        let mut reader = StructReader::new(bytes);

        while let Some(key) = reader.read_key()? {
            match AuctionCostsKeys::from_u64(key) {
                Some(AuctionCostsKeys::GetEraValidators) => {
                    auction_costs.get_era_validators = reader.read_value()?
                }
                Some(AuctionCostsKeys::ReadSeigniorageRecipients) => {
                    auction_costs.read_seigniorage_recipients = reader.read_value()?
                }
                Some(AuctionCostsKeys::AddBid) => auction_costs.add_bid = reader.read_value()?,
                Some(AuctionCostsKeys::WithdrawBid) => {
                    auction_costs.withdraw_bid = reader.read_value()?
                }
                Some(AuctionCostsKeys::Delegate) => auction_costs.delegate = reader.read_value()?,
                Some(AuctionCostsKeys::Undelegate) => {
                    auction_costs.undelegate = reader.read_value()?
                }
                Some(AuctionCostsKeys::RunAuction) => {
                    auction_costs.run_auction = reader.read_value()?
                }
                Some(AuctionCostsKeys::Slash) => auction_costs.slash = reader.read_value()?,
                Some(AuctionCostsKeys::Distribute) => {
                    auction_costs.distribute = reader.read_value()?
                }
                Some(AuctionCostsKeys::WithdrawDelegatorReward) => {
                    auction_costs.withdraw_delegator_reward = reader.read_value()?
                }
                Some(AuctionCostsKeys::WithdrawValidatorReward) => {
                    auction_costs.withdraw_validator_reward = reader.read_value()?
                }
                Some(AuctionCostsKeys::ReadEraId) => {
                    auction_costs.read_era_id = reader.read_value()?
                }
                Some(AuctionCostsKeys::ActivateBid) => {
                    auction_costs.activate_bid = reader.read_value()?
                }
                None => reader.skip_value()?,
            }
        }

        Ok((auction_costs, reader.finish()))
    }
}
