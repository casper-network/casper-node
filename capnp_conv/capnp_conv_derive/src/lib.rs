...
use self::{
    derive_enum::{gen_read_capnp_enum, gen_write_capnp_enum},
    derive_struct::{gen_read_capnp_named_struct, gen_write_capnp_named_struct},
    util::{assign_defaults_path, extract_defaults, remove_with_attributes},
};
...
pub fn capnp_conv(
    ...
    // Declaration corresponding capnp::traits::Owned associated type
    let corresponding_capnp_impl = quote! {
        impl ::capnp_conv::CorrespondingCapnp for #rust_struct {
            type Type = #capnp_struct::Owned;
        }
    };
    ...
    let conversion = match input.data {
        Data::Struct(ref data) => match data.fields {
            Fields::Named(ref fields_named) => {
                ...
                let write_capnp =
                    gen_write_capnp_named_struct(fields_named, rust_struct, &assign_defaults);
                let read_capnp =
                    gen_read_capnp_named_struct(fields_named, rust_struct, &assign_defaults);
        ...
        Data::Enum(ref data_enum) => {
            ...
            //     Type3(MyStruct),
            // }
            let write_capnp = gen_write_capnp_enum(data_enum, rust_struct, &assign_defaults);
            let read_capnp =
    ...
    let expanded = quote! {
        ...
        // Corresponding capnp::traits::Owned associated type
        #corresponding_capnp_impl
