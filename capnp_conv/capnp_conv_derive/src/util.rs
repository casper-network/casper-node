...
pub fn extract_defaults(generics: &syn::Generics) -> BTreeMap<syn::Ident, syn::Path> {
...
pub fn assign_defaults_path(path: &mut syn::Path, defaults: &BTreeMap<syn::Ident, syn::Path>) {
    ...
    if path.segments.len() == 1 {
        ...
        if let syn::PathArguments::None = last_segment.arguments {
            if let Some(default_path) = defaults.get(&last_segment.ident) {
                let _ = core::mem::replace(path, default_path.clone());
                return;
            }
        }
    }
