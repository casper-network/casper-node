use capnp_conv::{capnp_conv, FromCapnpBytes, ToCapnpBytes};
...
fn capnp_serialize_basic_struct() {
    let test_struct = TestStruct {
        ...
        inline_union: InlineUnion::Second(TestStructInner { inner_u8: 5u8 }),
        external_union: TestUnion::One(6u64),
    ...
    let data = test_struct.to_packed_capnp_bytes();
    let test_struct2 = TestStruct::from_packed_capnp_bytes(&data).unwrap();
...
fn capnp_serialize_floats() {
    ...
    let data = float_struct.to_packed_capnp_bytes();
    let float_struct2 = FloatStruct::from_packed_capnp_bytes(&data).unwrap();
...
fn capnp_serialize_generic_struct() {
    ...
    let data = generic_struct.to_packed_capnp_bytes();
    let generic_struct2 = GenericStruct::from_packed_capnp_bytes(&data).unwrap();
...
fn capnp_serialize_generic_enum() {
    for generic_enum in &[
        GenericEnum::A(1u32),
        GenericEnum::B(TestStructInner { inner_u8: 2u8 }),
        GenericEnum::C(3u64),
        GenericEnum::D(vec![1, 2, 3, 4u8]),
    ] {
        let data = generic_enum.to_packed_capnp_bytes();
        let generic_enum2 = GenericEnum::from_packed_capnp_bytes(&data).unwrap();
...
fn capnp_serialize_generic_list() {
    ...
    let data = list_generic.to_packed_capnp_bytes();
    let list_generic2 = ListGeneric::from_packed_capnp_bytes(&data).unwrap();
