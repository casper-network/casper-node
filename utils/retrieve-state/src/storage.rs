...
use casper_execution_engine::{
    ...
    storage::{global_state::db::DbGlobalState, trie_store::db::RocksDbStore},
};
use casper_node::{
    storage::Storage,
...
pub fn create_execution_engine(
    ...
    let global_state = DbGlobalState::empty(None, RocksDbStore::new(rocksdb_path.as_ref())?)?;
    Ok(Arc::new(EngineState::new(
        global_state,
        EngineConfig::default(),
    )))
