...
# Step 07: Check rpc backwards compatibility for the chain_get_era_info_by_switch_block RPC
# Step 08: Terminate.
...
function _step_01()
{
    ...
    log_step_upgrades 0 "Begin upgrade_scenario_12"
...
function _step_07()
{
    ...
    local ERA_INFO_RPC_RESPONSE=$(
        curl -s --header 'Content-Type: application/json' \
            ...
            --data-raw "{
                ...
                \"params\": {
                    \"block_identifier\": {
                        \"Hash\": $SWITCH_BLOCK_BEFORE_UPGRADE_HASH
                    }
                },
                \"id\": 1
            }" | jq '.result.era_summary'
    ...
    if [ "$ERA_INFO_RPC_RESPONSE" = "null" ]; then
        log "ERROR :: era summary could not be obtained through chain_get_era_info_by_switch_block for block $SWITCH_BLOCK_BEFORE_UPGRADE_HASH"
        exit 1
    fi
}
...
function _step_08()
{
    log_step_upgrades 8 "upgrade_scenario_12 successful - tidying up"
