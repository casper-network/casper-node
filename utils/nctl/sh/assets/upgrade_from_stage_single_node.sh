...
#   Whether to overwrite the node launcher even if it already exists (defaults to false)
#######################################
function _setup_asset_binaries()
{
    ...
    if [ "$FORCE_LAUNCHER_UPDATE" = "true" ]; then
            log "... forcing launcher update"
...
function _main()
{
    ...
    local VERBOSE=${3}
    local NODE_ID=${4}
    local FORCE_LAUNCHER_UPDATE=${5:-false}
    local PATH_TO_STAGE
    local PROTOCOL_VERSION
...
do
    ...
    case "$KEY" in
        ...
        node) NODE_ID=${VALUE} ;;
        force_launcher_upgrade) FORCE_LAUNCHER_UPDATE=${VALUE} ;;
        *)
    esac
done
