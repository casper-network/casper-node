...
#######################################
# COUNTER: Installs contract under network faucet account.
#######################################
function main()
{
    local CHAIN_NAME
    ...
    # Set contract path.
    PATH_TO_CONTRACT=$(get_path_to_contract "counter_define.wasm")
    if [ ! -f "$PATH_TO_CONTRACT" ]; then
        echo "ERROR: The counter_define.wasm binary file cannot be found.  Please compile it and move it to the following directory: $(get_path_to_net)"
        return
    fi
    ...
    log "installing contract -> counter"
