...
function main() 
{
    ...
    local PATH_TO_CONTRACT=$(get_path_to_contract "withdraw_bid.wasm")

    local BIDDER_SECRET_KEY=$(get_path_to_secret_key $NCTL_ACCOUNT_TYPE_NODE $BIDDER_ID)
    local BIDDER_ACCOUNT_KEY=$(get_account_key $NCTL_ACCOUNT_TYPE_NODE $BIDDER_ID)
    local BIDDER_MAIN_PURSE_UREF=$(get_main_purse_uref $BIDDER_ACCOUNT_KEY)
