...
function _main()
{
    ...
    if [ "$STAGE_SOURCE" == "local" ]; then
        source "$NCTL/sh/staging/set_stage_from_local.sh" stage="$STAGE_ID" version="$PROTOCOL_VERSION"
    elif [ "$STAGE_SOURCE" == "remote" ]; then
        source "$NCTL/sh/staging/set_stage_from_remote.sh" stage="$STAGE_ID" version="$PROTOCOL_VERSION"
    else
        source "$NCTL/sh/staging/set_stage_from_commit.sh" stage="$STAGE_ID" version="$PROTOCOL_VERSION" commit="$STAGE_SOURCE"
    fi
