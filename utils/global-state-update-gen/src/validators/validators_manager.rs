...
use casper_types::{
    system::{
        auction::{
            Bid, Bids, DelegationRate, SeigniorageRecipient, SeigniorageRecipientsSnapshot,
...
impl ValidatorsUpdateManager {
    ...
    pub fn perform_update(&mut self, validators: Vec<(&str, &str, Option<&str>)>) {
        ...
        let (input_data, new_snapshot) = gen_snapshot(
            validators,
        ...
        self.add_and_remove_bids(&input_data, &validators_diff, &new_snapshot);
...
type InputData = BTreeMap<PublicKey, (U512, DelegationRate)>;
...
fn gen_snapshot(
    validators: Vec<(&str, &str, Option<&str>)>,
    starting_era_id: EraId,
    count: u64,
) -> (InputData, SeigniorageRecipientsSnapshot) {
    ...
    for (pub_key_str, bonded_amount_str, maybe_delegation_rate) in &validators {
        ...
        let delegation_rate = maybe_delegation_rate
            .and_then(|s| DelegationRate::from_str(s).ok())
            .unwrap_or_default();
        input_data.insert(validator_pub_key.clone(), (bonded_amount, delegation_rate));
