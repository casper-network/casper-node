...
fn main() -> anyhow::Result<()> {
    ...
    match cli.command {
        Command::BuildSchema {
            ...
            let schema = match cli::build_schema::build_schema_impl(package_name) {
                Ok(schema) => schema,
            ...
            serde_json::to_writer_pretty(&mut schema_writer, &schema)?;

            eprintln!("✅ Schema generation completed successfully.");
        }
        Command::Build {
            output,
            workspace,
            ...
            let build_result =
                cli::build::build_impl(package_name, output, allow_skipping_abi_schema)?;
            ...
            if let Some(meta_path) = build_result.meta {
                println!("Meta built at: {}", meta_path.display());
            }
        }
        Command::New { name } => cli::new::new_impl(&name)?,
        Command::VerifyMeta { meta, schema, wasm } => {
            cli::verify_meta::verify_meta_impl(meta, schema, wasm)?;
        }
    }
    Ok(())
}
