...
impl MessageEmitter {
        ...
        T: StateReader<Key, StoredValue, Error = Error>,
    {
        ...
        let Some(StoredValue::MessageTopic(prev_topic_summary)) = tracking_copy
            .read(&topic_key)
            .map_err(|err| MessageEmissionError::TrackingCopy(err))?
        else {
        ...
        let topic_message_index = if prev_topic_summary.blocktime() != current_blocktime {
            for index in 1..prev_topic_summary.message_count() {
                tracking_copy.prune(Key::message(entity_addr, topic_name_hash, index).into());
            }
            0
        } else {
...
pub(crate) struct NewContractMessagesEmitter {
    /// This should be the Key::Hash with addr of the ContractPackage (if addressable entity is disabled) or Key::SmartContract otherwise
    key_of_package: Key,
    /// This should be the Key::Hash with addr of the Contract (if addressable entity is disabled) or Key::AddressableEntity(EntityAddr::SmartContract()) otherwise
    key_of_contract: Key,
    /// This should be the Key::Hash with addr of the ContractWasm (if addressable entity is disabled) or Key::Bytecode otherwise
    key_of_wasm: Key,
