...
impl EvmPredeploy {
    fn eip4788() -> Self {
        EvmPredeploy {
            name: "EIP-4788",
            address: evm::BEACON_ROOTS_ADDRESS,
            code: evm::BEACON_ROOTS_CODE,
            code_hash: evm::beacon_roots_code_hash(),
        }
    }
    ...
    fn eip2935() -> Self {
        EvmPredeploy {
            name: "EIP-2935",
            address: evm::BLOCK_HASH_HISTORY_ADDRESS,
            code: evm::BLOCK_HASH_HISTORY_CODE,
            code_hash: evm::block_hash_history_code_hash(),
        }
    }
...
/// Idempotently installs Prague EVM predeploys.
...
mod tests {
    ...
    #[test]
    fn upsert_creates_missing_predeploys() {
    ...
    #[test]
    fn upsert_noops_when_predeploys_are_present() {
    ...
    fn upsert_rejects_unexpected_eip2935_code_hash_value() {
        ...
        let (mut tracking_copy, _tempdir) = tracking_copy([(
            predeploy.code_hash_key(),
            StoredValue::CLValue(
                CLValue::from_t(Key::Evm(EvmAddr::Account(evm::BLOCK_HASH_HISTORY_ADDRESS)))
